...
The corrective parity checks are measurements of commuting Paulis such as $Z_1Z_2$. The group they
generate is the \textbf{stabilizer} $S$, an Abelian subgroup of $\mathcal P_n$ with $-1\notin S$ and
$M\ket{\psi}=\ket{\psi}$ for all $M\in S$, $\ket{\psi}\in C$. If an error $E$ anticommutes with a
generator $M$, then
\[ M\,(E\ket{\psi}) = -E M\ket{\psi} = -E\ket{\psi}, \]
flipping the eigenvalue to $-1$. Measuring the generators gives the \textbf{error syndrome}, which
identifies $E$ up to degeneracy $E^\dagger F\in S$.

\begin{theorem*}[Stabilizer code]
If $S$ is an Abelian subgroup of order $2^a$ of $\mathcal P_n$ with $-1\notin S$, and $d$ is the
minimum weight in $N(S)\setminus S$ (where $N(S)$ is the normalizer), then the states fixed by $S$
form an $[[n,n-a,d]]$ code.
\end{theorem*}

\noindent In the \textbf{binary symplectic representation} each Pauli is a pair $(a\mid b)$; two
operators commute iff $a_1\!\cdot\!b_2 + b_1\!\cdot\!a_2 = 0$, and the stabilizer condition becomes
self-orthogonality under this symplectic form.
...
\begin{itemize}[leftmargin=*]
  ...
  \item The stabilizer formalism turns codes into Abelian Pauli groups and syndromes into eigenvalue
  flips, linking quantum codes to classical and $\mathrm{GF}(4)$ coding theory.
